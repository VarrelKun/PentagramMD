...
let anu =`
${pickRandom(global.heker)}
`
     const message = {
            document: { url: thumb },
            jpegThumbnail: await (await fetch(thumb)).buffer(), fileName: '─────〔 𝙃𝘼𝘾𝙆𝙀𝙍 〕─────', mimetype: dpdf, fileLength: fsizedoc, pageCount: fpagedoc, giflogo,
            caption: wm,
            footer: anu,
            templateButtons: [
                {
...
handler.help = ['hacker']
handler.tags = ['quotes']
handler.command = /^(heker|hengkel|heker|hacker)$/i
handler.group = false
handler.limit = true
